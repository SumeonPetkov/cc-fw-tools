...
if (($# != 1)); then
  echo "Usage: ./unpack2.sh update_file"
  echo "Example: ./unpack2.sh FW/AC104_K2Pro_1.1.0_3.0.5_update.bin"
  exit 1
fi
...
if [ "$UPDATE_FILE_EXT" != "bin" ] && [ "$UPDATE_FILE_EXT" != "swu" ] && [ "$UPDATE_FILE_EXT" != "zip" ] && [ "$UPDATE_FILE_EXT" != "sig" ]; then
...
DECRYPT_TOOL="TOOLS/cc2_swu_decrypt.py"

# directory to unpack to
UNPACK_DIR=unpacked2
...
if [ "$UPDATE_FILE_EXT" == "bin" ]; then
  ...
  $DECRYPT_TOOL -i "$UPDATE_FILE" -o ./${UNPACK_DIR}/update.zip
  if [ ! -f "./${UNPACK_DIR}/update.zip" ]; then
    echo -e "${RED}ERROR: Cannot find the input file './${UNPACK_DIR}/update.zip' ${NC}"
    exit 4
  fi
  cd ${UNPACK_DIR} || exit 5
  unzip update.zip
  rm -r update.zip
  cd ..
else
  if [ "$UPDATE_FILE_EXT" == "zip" ]; then
    # zip file
    cp "$UPDATE_FILE" ./${UNPACK_DIR}/update.zip
    cd ${UNPACK_DIR} || exit 5
    unzip update.zip
    rm -r update.zip
    cd ..
    ...
    mkdir ./${UNPACK_DIR}/update
    cp "$UPDATE_FILE" ./${UNPACK_DIR}/update/update.swu
  fi
fi

# check the input file
if [ ! -f "./${UNPACK_DIR}/update/update.swu" ]; then
  echo -e "${RED}ERROR: Cannot find the input file './${UNPACK_DIR}/update/update.swu' ${NC}"
  exit 6
fi

# extract the update
cd ${UNPACK_DIR} || exit 7
...
echo ""
echo -e "${GREEN}Unpacking DONE! Check the '${UNPACK_DIR}' folder for the result.${NC}"
echo ""
